...
export function UserPreferenceScreen() {
    ...
    const getPlaceId = async (placeName, cityName) => {
    ...
    const FAKE_REVIEWS = [
        {
            ...
            rating: 5,
            text: "Great neighbourhood! Quiet, clean, and perfect for evening walks.",
        },
        {
            author_name: "Aman Verma",
            rating: 4,
            text: "Good for students. Affordable rent and lots of food joints nearby.",
        },
        {
            author_name: "Neha Kapoor",
            rating: 3,
            text: "Nice place but can get crowded during weekends.",
        },
        {
            author_name: "Prateek Jain",
            rating: 5,
            text: "Loved the greenery and parks! Great for families.",
        },
        {
            author_name: "Sana Sheikh",
            rating: 4,
            text: "Safe and well-connected by public transport.",
        },
    ];
    ...
    useEffect(() => {
        if (selectedNeighbour?.name) {
            ...
            setTimeout(() => {
                // Pick 3 fake reviews randomly
                const shuffled = [...FAKE_REVIEWS].sort(() => 0.5 - Math.random());
                setReviews(shuffled.slice(0, 3));
                setLoadingReviews(false);
            }, 1200); // Simulate a short fake delay
        }
    }, [selectedNeighbour]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                fetchGoogleReviews(data[0].name, cityName);
            } else {
    ...
    useEffect(() => {
        if (selectedNeighbour?.name) {
            fetchGoogleReviews(selectedNeighbour.name, cityName);
        }
    }, [selectedNeighbour]);
    ...
    return (
        ...
        >
            <section id="home" className="bg-white min-h-screen w-full">
                <div className="mt-6">{error && <AlertError error={error} />}</div>
                ...
                <div className="max-w-screen-xl mx-auto px-6 py-10 min-h-screen mb-6 overflow-x-hidden">
                    ...
                    {/* Sliders and carousel form section here */}
                                    ...
                                    {top3Neighborhoods.map((n, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {index === 0 && (
                                                    <span className="mt-4 inline-block bg-black text-white px-3 py-1 text-xs rounded-full">
                                                        Top Pick
                                                    </span>
                                                )}
                        ...
                        {selectedNeighbour?.name && (
                            <div className="mt-12">
                                ...
                                {loadingReviews ? (
                                    ...
                                    <div className="mt-8">
                                        <h3 className="text-xl font-semibold mb-4 text-gray-800">Google Reviews</h3>
                                        <ul className="space-y-4">
                                            {reviews.slice(0, 3).map((rev, idx) => (
                                                <li key={idx} className="border bg-white rounded-xl p-4 shadow-sm hover:shadow-md transition">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <p className="font-medium text-gray-700">{rev.author_name}</p>
                                    ...
                                    <p className="text-sm text-gray-500 mt-4">No reviews found for this location.</p>
                                )}
